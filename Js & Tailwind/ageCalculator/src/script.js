  ...
  try {
    ...
    if (data.data && data.data.Events && data.data.Events.length > 0) {
      ...
      return shuffledEvents
        .slice(0, Math.min(4, shuffledEvents.length))
        .map((event) => {
          return {
...
async function getWikipediaEvents(month, day) {
  try {
    const url = `https://en.wikipedia.org/api/rest_v1/feed/onthisday/events/${month
      .toString()
...
function getSpecialDateEvents(month, day) {
  const specialDates = {
    "09-11": [
      {
        year: 2001,
        event:
          "Terrorist attacks occurred in New York City and Washington D.C., killing nearly 3,000 people",
        category: "😢 Tragedy",
      },
      {
        year: 1973,
        event:
          "Chilean President Salvador Allende was overthrown in a military coup led by Augusto Pinochet",
        category: "🏛️ Politics",
      },
      {
        year: 1922,
        event: "The British Mandate for Palestine officially began",
        category: "📜 Treaty",
      },
      {
        year: 1941,
        event:
          "Ground was broken for the construction of the Pentagon in Arlington, Virginia",
    ...
    "07-20": [
      ...
      {
        year: 1944,
        event: "The July 20 plot to assassinate Adolf Hitler failed",
        category: "⚔️ War",
      },
      {
        year: 1871,
        event: "British Columbia joined the Canadian Confederation",
        category: "🏛️ Politics",
      },
    ],
    "12-25": [
      {
        year: 0,
        event: "Jesus Christ was born according to Christian tradition",
        category: "⛪ Religion",
      },
      {
        year: 1776,
        event:
          "George Washington crossed the Delaware River during the American Revolutionary War",
        category: "⚔️ War",
      },
      {
        year: 1991,
        event: "The Soviet Union was officially dissolved",
        category: "🏛️ Politics",
      },
    ],
    "01-01": [
      {
        year: 2000,
        event:
          "The Y2K millennium bug failed to cause major computer disruptions worldwide",
        category: "💻 Technology",
      },
      {
        year: 1863,
        event: "Abraham Lincoln issued the Emancipation Proclamation",
        category: "⚖️ Rights",
      },
      {
        year: 1959,
        event: "Fidel Castro's revolutionary forces captured Havana, Cuba",
        category: "🏛️ Politics",
      },
    ],
  };

  const dateKey = `${month.toString().padStart(2, "0")}-${day
    .toString()
    .padStart(2, "0")}`;
...
function getGenericBackupEvents() {
  const backupEvents = [
    {
      year: 1969,
      event: "Apollo 11 successfully landed on the Moon",
      category: "🚀 Space",
    },
    {
      year: 1989,
      event: "The Berlin Wall fell, reuniting East and West Germany",
      category: "🏛️ Politics",
    },
    {
      year: 1776,
      event: "The United States Declaration of Independence was signed",
      category: "📜 Treaty",
    },
    {
      year: 1945,
      event: "World War II ended with Japan's surrender",
      category: "⚔️ War",
    },
    {
      year: 1955,
...
function getCategoryFromEvent(eventText) {
  ...
  if (
    ...
    return "🚀 Space";
  if (text.includes("earthquake") || text.includes("disaster"))
    return "🌍 Natural";
  if (text.includes("independence") || text.includes("treaty"))
    return "📜 Treaty";
...
async function displayHistoricalEvents(month, day) {
  ...
  try {
    ...
    container.innerHTML = events
      .map(
        (event, index) => `
                    <div class="history-card glass-effect p-4 rounded-xl border border-amber-200/20 animate-slideInLeft" style="animation-delay: ${
                      index * 0.1
                    }s;">
...
function exactAge() {
  ...
  if (window.innerWidth >= 768 && birthInput.value) {
    ...
    result.innerHTML =
      '<span class="text-red-300">Please select your birth date first</span>';
    return;
  }
...
`;
  ...
  if (years < 0) {
    ...
    commentLine.textContent =
      "Bro… you haven't even taken your first breath. 💀";
  } else if (years === 0 && months === 0) {
    commentLine.textContent =
      "Fresh out the oven! Welcome to the world, tiny human. 👶✨";
  } else if (years === 0) {
    commentLine.textContent = "Baby mode activated: milk, sleep, repeat. 🍼😴";
  } else if (years >= 1 && years <= 2) {
    commentLine.textContent =
      "Toddler mode: drool, tantrums, and mystery stains. 🍼🧩";
  } else if (years >= 3 && years <= 4) {
    commentLine.textContent =
      "You think everything is 'yours'—including my phone. 😅📱";
    ...
    commentLine.textContent =
      "You believe you're invincible—until homework says otherwise. 📚⚡";
  } else if (years >= 11 && years <= 12) {
    commentLine.textContent =
      "Secretly googling how to ask your crush out. 🕵️‍♂️💕";
  } else if (years >= 13 && years <= 14) {
    commentLine.textContent =
      "Hormones raging—welcome to the mood swing rollercoaster. 🎢😭";
  } else if (years >= 15 && years <= 16) {
    commentLine.textContent =
      "Driver's permit dreams & pizza-stained selfies. 🚗🍕";
  } else if (years >= 17 && years <= 18) {
    commentLine.textContent =
      "Late-night study sessions fueled by regret and energy drinks. 📖⚡";
  } else if (years >= 19 && years <= 20) {
    commentLine.textContent = "Ramen gourmet or broke college legend? 🍜🏆";
  } else if (years >= 21 && years <= 22) {
    commentLine.textContent =
      "Legal adult… still living on instant noodles and memes. 😂";
  } else if (years >= 23 && years <= 24) {
    commentLine.textContent =
      "'What do I want to do with my life?' starter pack engaged. 🤷‍♂️🎯";
  } else if (years >= 25 && years <= 26) {
    commentLine.textContent =
      "Quarter-life crisis with a side of avocado toast. 🥑🍞";
  } else if (years >= 27 && years <= 28) {
    commentLine.textContent =
      "Job title sounds fancy; paycheck says 'LOL'. 💼💸";
  } else if (years >= 29 && years <= 30) {
    commentLine.textContent =
      "Happy 30th! Time to start using anti-aging cream… or not. 🎂🧴";
  } else if (years >= 31 && years <= 32) {
    commentLine.textContent = "Back pain & midweek naps coming in hot. 🛌🔥";
  } else if (years >= 33 && years <= 34) {
    commentLine.textContent =
      "'When I was your age…' about to become your catchphrase. 📖👴";
  } else if (years >= 35 && years <= 36) {
    commentLine.textContent =
      "Birthday cake with kale frosting? Adulting hurts. 🥬🎂";
  } else if (years >= 37 && years <= 38) {
    commentLine.textContent =
      "Weekend plans: laundry, taxes, existential dread. 🧺📜";
  } else if (years >= 39 && years <= 40) {
    commentLine.textContent =
      "40's club: creaky knees & wisdom teeth regrets. 🦴🦷";
  } else if (years >= 41 && years <= 42) {
    commentLine.textContent =
      "Your phone autocorrects 'vibe' to 'tribe'—it's watching you. 📱👀";
  } else if (years >= 43 && years <= 44) {
    commentLine.textContent =
      "You've seen fashion trends come back… and go back again. 👖🔄";
  } else if (years >= 45 && years <= 46) {
    commentLine.textContent = "'Kids these days' lectures incoming. 📣🛑";
  } else if (years >= 47 && years <= 48) {
    commentLine.textContent =
      "You laugh at jokes before they finish telling them. 😂⏳";
    ...
    commentLine.textContent =
      "Your playlist is 50% golden oldies, 50% 'what is this?' 🎶❓";
    ...
    commentLine.textContent =
      "Celebrating six decades of epic life hacks. 🎂🛠️";
  } else if (years >= 61 && years <= 62) {
    commentLine.textContent =
      "You've unlocked 'wisdom of the ages' status. 🧙‍♂️✨";
    ...
    commentLine.textContent =
      "You've seen eras change—got wisdom to spare. 🕰️💡";
  } else if (years >= 97 && years <= 98) {
    commentLine.textContent =
      "Legendary elder: stories sell out stadiums. 🎤🏟️";
  } else if (years >= 99 && years <= 100) {
    commentLine.textContent =
      "Centennial boss—100 years and still slaying! 💯👊";
  } else {
    commentLine.textContent =
      "Over 100? You're basically a mythic deity now. 🏰⚡";
  }
...
function initializeThemeToggle() {
  ...
  const lightIcons = document.querySelectorAll(
    "#theme-toggle-light-icon, #mobile-theme-toggle-light-icon"
  );
  const darkIcons = document.querySelectorAll(
    "#theme-toggle-dark-icon, #mobile-theme-toggle-dark-icon"
  );
  ...
  if (refreshHistoryBtn) {
    refreshHistoryBtn.addEventListener("click", function () {
      ...
      if (birthDate) {
        ...
        if (container) {
          container.innerHTML =
            '<div class="text-center text-white"><div class="text-2xl animate-spin">⚡</div><p class="mt-2">Finding more historical events...</p></div>';
          setTimeout(() => {
            displayHistoricalEvents(month, day);
          }, 800);
        }
